...
/**
 * A grid of dots that bulges away from the cursor.
 *
 * The loop stops once every dot is back on its anchor and restarts on the
 * next mousemove, so a visitor reading gets one static frame, not 60 redraws
 * a second. Under reduced motion the cursor is never tracked.
 */
export const DotField = memo(function DotField({
  ...
  sparkle = false,
  waveAmplitude = 0,
  gradientFrom = "rgba(61, 176, 228, 0.35)",
  gradientTo = "rgba(241, 235, 251, 0.25)",
  glowColor = "#040038",
  ...rest
}: DotFieldProps) {
  ...
  // useId, not Math.random: the id is rendered on the server too.
  ...
  useEffect(() => {
    ...
    // The document never scrolls (see .scroll-shell), so client coordinates are page coordinates.
    ...
    function tick() {
      ...
      const animated = (p.waveAmplitude as number) > 0 || (p.sparkle as boolean);
      let maxOffset = 0;
      ...
      for (let i = 0; i < len; i++) {
        ...
        maxOffset = Math.max(maxOffset, Math.abs(d.sx - d.ax) + Math.abs(d.sy - d.ay));
      ...
      const settled = !animated && eng === 0 && glowOpacity.current === 0 && maxOffset < 0.05;
      raf = settled || reduced ? null : requestAnimationFrame(tick);
    }

...
import { ScrollTrigger, scroller, useGSAP } from "@/lib/gsap";
        ...
        <Container className="relative grid gap-12 pb-12 pt-header lg:grid-cols-[minmax(0,1.35fr)_minmax(0,0.65fr)] lg:items-end lg:gap-16">
          ...
          >
            <p className="max-w-sm text-sm leading-7 text-on-detail">
              Software a medida y agentes de IA integrados. La solución queda
              siendo tuya.
            </p>
            {/* Into the site, not out of it. This was a link to
                novitsoftware.com, which sent anyone who opened the menu
                straight off it. Root-relative for the same reason the nav
                items are: the menu is mounted on every route, and a bare
                fragment resolves to nothing from /academianovit. */}
            <a
              href="/#contacto"
              onClick={close}
              className="link-rule w-fit text-xs font-bold uppercase tracking-[0.16em] text-celeste"
            >
              Solicitar cotización
            </a>
            {/* The direct channels, below the page's own CTA rather than
                instead of it: this is for someone who already knows what
                they want to say. */}
...
/**
 * Tells the menu which section the viewport is sitting in. The triggers are
 * created inside a `useGSAP` context, which kills them on unmount.
 */
function useActiveSection(onChange: (id: string) => void) {
  useGSAP(() => {
    // `item.id`, not `item.href.slice(1)`: the hrefs are root-relative now
    // (`/#servicios`), so slicing one gives `#servicios` and every lookup
    // missed silently — no active state anywhere on the page. The Academia's
    // entry points at a route rather than a section, so it simply has no
    // element to find here and is skipped.
...
function useMenuAccessibility({
  ...
  useEffect(() => {
    ...
    // `#pie`, not `#contacto`: that anchor belongs to the contact section now,
    // and the contact section is inside `#contenido`. Looking the footer up by
    // the old id would have left the whole footer tabbable behind the open
    // menu — silently, because `?.` on a miss does nothing.
